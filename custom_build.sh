...
make_checks() {
    ...
    echo "########################"
    cd ${out_dir}
    sha1sum ${iso_name}-${iso_version}-x86_64.iso > ${iso_name}-${iso_version}-x86_64.sha1
    echo "Building sha256sum"
    echo "########################"
    sha256sum ${iso_name}-${iso_version}-x86_64.iso > ${iso_name}-${iso_version}-x86_64.sha256
    echo "Building md5sum"
    echo "########################"
    md5sum ${iso_name}-${iso_version}-x86_64.iso > ${iso_name}-${iso_version}-x86_64.md5
    echo "Moving pkglist.x86_64.txt"
    echo "########################"
    cd ..
    cp ${work_dir}/iso/arch/pkglist.x86_64.txt  ${out_dir}/${iso_name}-${iso_version}-x86_64.iso.pkglist.txt
}
